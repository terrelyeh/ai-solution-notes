...
- Frontend / Admin：Next.js + React + TypeScript，部署在 Vercel
- Backend：Vercel 上的 Next.js Route Handler / API Routes
- Database：Supabase Postgres
- Vector Search：Supabase pgvector
- File Storage：Supabase Storage
- LLM：OpenAI / Claude / Gemini 任一 provider，需包成 adapter
- Queue / Jobs：QStash、Supabase Edge Functions、Vercel Cron，進階可用 BullMQ 或 Cloudflare Queues
- Notification：Slack、Email、LINE 內部通知 bot 任一

## 4.1 技術架構與資料庫選型

LINE OA Agentic Workflow 不是純前端網站，必須有後端 server 或 serverless backend。建議以 Vercel 作為第一版部署平台：Vercel 負責 Next.js web app、LINE webhook、API routes、客服後台與短時間 orchestration；Supabase 負責 Postgres、pgvector、Storage、Auth；長時間工作交給 QStash / Supabase Edge Functions / background jobs。
...
→ Vercel /api/line/webhook
  ...
  → Message Logging
  → Orchestrator Service on Next.js
    → Router / Workflow State
    → RAG Retriever
    → Tool Executor
    → Human Handoff
  → Supabase Postgres / pgvector / Storage
  → Queue or Background Jobs for slow tasks
→ LINE Reply or Push API
```

### Vercel Backend / Serverless 選項

| 選項 | 適合情境 | 優點 | 注意事項 |
|---|---|---|---|
| Vercel + Next.js Route Handler / API Routes | MVP、中小型正式產品 | GitHub 自動部署、前台、後台、API 同 repo，開發快 | 長任務需要 queue，不要卡 webhook |
| Vercel Edge Functions | 簡單低延遲 API | 靠近使用者、回應快 | Node API 與部分套件有限制，不適合重型處理 |
| Cloudflare Workers | 更重視低延遲 webhook 或全球 edge | 快、便宜、部署簡單 | 會變成另一個部署平台，複雜後台較不直覺 |
...
建議 MVP：`Vercel + Next.js + Supabase`。如果未來 webhook 延遲要求很高，再評估把特定 webhook edge layer 拆到 Cloudflare Workers。
...
- `/api/line/webhook`
- `/api/admin/*`
- Orchestrator service 的短流程
- RAG retrieval
- 建立 ticket
- 寫入 audit log
- 呼叫 LINE Reply / Push API

不建議直接放在 Vercel request 裡長時間執行：

- PDF / Word 大量文字抽取
- 大量 embedding
- Notion / Obsidian 全量同步
- 長時間 reindex
- 很慢的第三方 API 操作
- 批次報表

這些工作應該丟給 queue 或 background job。
...
LINE webhook 應該快速回應，不應等待慢任務完成。Vercel function 負責收訊息、寫 log、快速決策；慢任務應進 queue 或 background job：
...
| 選項 | 適合情境 |
|---|---|
| QStash | Serverless queue，最適合 Vercel 架構 |
| Supabase Edge Functions / Cron | 簡單背景任務、資料庫附近的 job |
| Vercel Cron | 定期觸發 reindex / sync，但不應承擔重型工作本身 |
| Cloudflare Queues | Workers 架構 |
| BullMQ + Redis | 複雜 job pipeline、自架 Node worker |

MVP 可以先用 QStash 或 Supabase Edge Functions，但架構上要預留 queue。
...
→ Supabase pgvector
→ Supabase Storage
→ QStash / Supabase Edge Functions for slow jobs
